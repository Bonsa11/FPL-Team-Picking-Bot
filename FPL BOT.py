...
possible_players = possible_players[possible_players['status'] == 'a']
...
def build_initial(initial_team):
    ...
    if ((len(initial_team) < 6) and (len(team_FWDS) > formation[3]) and (len(team_MIDS) > formation[2]) and (len(team_DEFS) > formation[1]) and (len(team_GKPS) > formation[0])):
        return(build_initial(initial_team))
    else:
        return(initial_team)

initial_team = build_initial(initial_team)
...
if remaining_budget > 0:
    print('budget is good ', remaining budget, ' left in the bank')
else
